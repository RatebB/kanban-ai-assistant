...
export function Column({ column, onCardAdded,  onBreakdown, onDelete  }: Props) {
  ...
  return (
   <div className="bg-white/60 rounded-lg p-3 w-72 flex-shrink-0 border border-[#E5E3DD]">
    ...
    <h3 className="text-[13px] font-semibold text-[#1A1A17] tracking-tight">
        {column.name}
        <span className="text-[13px] text-[#5B5850] font-semibold px-1">
      {column.cards.length}
    </span>
      </h3>
  </div>
      <div ref={setNodeRef} className="flex flex-col gap-2 min-h-[40px]">
        <SortableContext items={cardIds} strategy={verticalListSortingStrategy}>
          {column.cards.map((card) => (
            <Card key={card.id} card={card} onBreakdown={onBreakdown} onDelete={onDelete }/>
          ))}
        </SortableContext>
        <AddCardButton
            columnId={column.id}
